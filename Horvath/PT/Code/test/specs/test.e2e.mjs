    ...
    describe(`Scenario: ${scenarioName}`, () => {
      it("should complete automobile insurance quote flow", async () => {
        ...
        await browser.execute((gender) => {
          document
            .querySelector(`input[name="Gender"][value="${gender}"]`)
            .click();
        }, scenario.gender);
        ...
        const formattedInsuranceSum = ` ${(
          scenario.insuranceSum / 1000000
        ).toFixed(0)}.000.000,00`;
        await $("#insurancesum").selectByVisibleText(formattedInsuranceSum);
        await $("#meritrating").selectByVisibleText(scenario.meritRating);
        await $("#damageinsurance").selectByVisibleText(
          scenario.damageInsurance
        );
